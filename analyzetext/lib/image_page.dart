...
class _CropPageState extends State<CropPage> with SingleTickerProviderStateMixin{
  ...
  _requestYandex(String image ){
    String folderId ="b1gfu90f6qhgluv8t9fg";
    String languageCode = "*"; //"ru" //"en"
    String image_base64 = "";
    image_base64 = image;
    Map<String, Object> tt =  {
          "folderId":  folderId, //"b1gvmob95yysaplct532",
          "analyze_specs": [{
              "content": image_base64,//"iVBORw0KGgo...",
              "features": [{
                  "type": "TEXT_DETECTION",
  ...
  Future<void> _capturePng() async {
    ...
    // await Future.delayed(const Duration(microseconds: 20), () async {});
    //TODO: errore
    // var boundary2 = globalKey.currentContext!.findRenderObject();
    // RenderRepaintBoundary boundary = RenderRepaintBoundary();
    // boundary.setupParentData(boundary2!);
    // boundary2.applyPaintTransform(boundary, pixelRatio);
    double pixelRatio = 1.5;
    ///Задержка для анимации нажатия
    
    //TODO: errore
    //_AssertionError ('package:flutter/src/rendering/proxy_box.dart': Failed assertion: line 3158 pos 12: '!debugNeedsPaint': is not true.)
    ui.Image image = widget.image!; //await boundary.toImage();
    ...
    var codec = await ui.instantiateImageCodec(Uint8List.fromList(byteList));
    var frame = await codec.getNextFrame();

    // _postImageBaidu( frame, base64Image);
    _postImageYandex( frame, base64Image);
  ...
  _postImageYandex(frame, String base64Image) async{
    Dio dio = new Dio();
    // String tokenUrl = baiduApi+"grant_type="+baiduGrantType+"&client_id="+baiduClientId+"&client_secret="+baiduClientSecret;
    ...
    Response res = await dio.post(
      ocrUrl,
    data:_requestYandex(base64Image),
    //{"image":base64Image}, 
    options: new Options(
      headers:header ,
    ...
    // int wordsNum =  int.parse(res.data["words_result_num"].toString());
    print("Result "+res.data.toString() +" "+ res.statusMessage.toString()+" "+ res.toString());
    List<String> ocrContent =  new List.empty(growable: true);
    if(true){
  ...
  // _postImageBaidu(frame, String base64Image) async{
  //   Dio dio = new Dio();
  //   String tokenUrl = baiduApi+"grant_type="+baiduGrantType+"&client_id="+baiduClientId+"&client_secret="+baiduClientSecret;
